...
test('新八字记录显式保存 Phase 2 深度快照而不是只保存展示摘要', async () => {
  ...
  assert.equal(saved.interpretationVersion, 'bazi-rules-v3');
  ...
  const persisted = JSON.parse(__storage.get(READINGS_KEY)).value[0];
  assert.equal(persisted.interpretationSnapshot.interpretationVersion, 'bazi-rules-v3');
  assert.equal(persisted.evidenceGraphSnapshot.evidenceVersion, 'bazi-evidence-v2');
