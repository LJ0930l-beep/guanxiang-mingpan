...
| P1-A 设置/证据骨架 | 已完成第一版 | `BaziCalculationSettings` 固定时区、日界、真太阳时模型、位置数据版本、历法解析版本；`BaziCalculationEvidence` 显式记录民用时刻、有效时刻、节气状态、日界和警告。具体日界与真太阳时规则已由 P1-C/P1-D 接管。 |
...
| P1-D 地点/真太阳时 | 已完成第一版 | `true-solar-time.ts` 用 UTC-only 算术实现地方平太阳时/视太阳时；标准经线固定 120°E，记录经度、均时差/经度修正、精度、有效时刻与数据版本；UI 可切换模型；已加入跨 `TZ=UTC`/`Asia/Shanghai` 回归。城市表已升级为带 `locationId`、来源、许可和版本的大陆首发离线索引。 |
| P1-E 农历/闰月/UI | 已完成第一版 | `calendar-resolver.ts` 固定 `lunar-javascript@1.7.7`，显式校验农历日期/闰月组合并转换公历；证据记录输入、换算结果、来源和 resolver 版本；跨春节、非法闰月、无效日期与跨 TZ fixtures 已加入；结果页已提供可展开的完整“本次计算依据”。 |
| P1-F 收口 | 已完成第一版 | 32 项回归、旧记录迁移、跨 TZ、future schema 写保护、城市精确匹配、文档和 Web CI 均已接入；全国地级市完整覆盖、夏令时公开样例、流派选择和 iPhone 真机验收仍是上线前工作。 |
...
- 当前默认真太阳时关闭，`trueSolarTime` 开启后必须选择 `localMeanSolarTime` 或 `apparentSolarTime`；修正后的有效时刻、经度和标准经线会写入证据，农历输入先换算为公历后再进入同一 UTC-only 修正路径。
