...
| 本地数据控制 | 已完成基线 | 命主支持编辑、删除及关联记录清理；记录支持删除与清空；设置页支持带版本 JSON 备份导出/导入；删除、清空与恢复均有二次确认，Storage 写保护会阻止不兼容数据被覆盖。 |
...
| M2：本地档案与备份 | 70% | 命主与结果自动归档、记录展开查看、版本字段、快照元数据、Storage Schema Version 与首版迁移；命主/记录编辑删除与清空；带版本 JSON 备份导出/导入。 | 收藏、反馈复盘、密码加密备份、冲突合并与完整迁移策略。 |
...
- `npm run typecheck`
- `npm run lint`
- `npm test`（四模块固定样例、缺失时辰、跨 TZ 六爻复现、真实用户写操作写保护与本机备份格式，共 14 项测试）
...
1. ~~实现命主与记录的编辑、删除、清空与二次确认。~~ 已完成首版本地控制；待补收藏、反馈复盘和更细的操作审计。
