...
| 本地数据控制 | 已完成基线 | 命主支持编辑、删除及关联记录清理；记录支持收藏筛选、按日事实反馈、删除与清空；设置页支持带版本普通 JSON 与密码保护加密备份导出/导入；删除、清空与恢复均有二次确认，Storage 写保护会阻止不兼容数据被覆盖。 |
...
- **八字可信度 P1-A**：`src/domains/bazi/` 维护独立来源 Golden Case、八字规则设置和 `BaziCalculationEvidence`。当前设置字段已进入快照，但节气边界、子初换日和真太阳时仍是明确的未启用状态；旧八字记录迁移时会标为“历史默认规则”，不静默重算。
...
- 已有用户可操作的普通 JSON 与密码保护加密备份导出、导入和清除全部本地数据；加密备份的密码不落盘且无法找回，跨版本迁移和冲突合并策略仍需补齐。
...
| M1：确定性排盘核心 | 85% | 四类本地排盘、统一结果协议、输入边界、独立来源 Golden Case；P1-A 设置/证据骨架与 P1-B 节气边界（来源、版本、精度、前后节气、月柱依据、跨 TZ）已落地。 | 全国城市库、真太阳时/子初换日/流派设置、农历/闰月/子初/夏令时等完整 Golden 回归测试。 |
...
- `npm run typecheck`
- `npm run lint`
- `npm test`（四模块固定样例、P1-A 独立来源 Golden Case、P1-B 节气边界 T-1/T/T+1 与跨 TZ、缺失时辰、六爻复现、真实用户写操作写保护、收藏/反馈持久化、普通/加密本机备份格式，共 24 项测试）
...
2. 在现有普通 JSON 与密码加密备份之上完成设备迁移冲突策略，并在 Web/iPhone 真机验收文件导出、分享与恢复。
