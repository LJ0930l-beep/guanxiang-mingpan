...
|---|---|---|---|
| 八字 | 已可用 | 年/月/日/时四柱、十神、藏干、纳音、空亡、部分柱间关系及基础观察；支持午夜/子初日界线并记录有效计算时刻。 | 必须提供精确出生时辰与性别；尚无真太阳时和流派设置。 |
...
- **八字可信度 P1-A～P1-C**：`src/domains/bazi/` 维护独立来源 Golden Case、节气边界解析、午夜/子初日界线、八字规则设置和 `BaziCalculationEvidence`。真太阳时仍明确未启用；旧八字记录迁移时会标为“历史默认规则”，不静默重算。
...
| M1：确定性排盘核心 | 88% | 四类本地排盘、统一结果协议、输入边界、独立来源 Golden Case；P1-A 设置/证据骨架、P1-B 节气边界和 P1-C 午夜/子初日界线已落地。 | 全国城市库、真太阳时/流派设置、农历/闰月/夏令时等完整 Golden 回归测试。 |
...
- `npm run typecheck`
- `npm run lint`
- `npm test`（四模块固定样例、P1-A 独立来源 Golden Case、P1-B 节气边界 T-1/T/T+1 与跨 TZ、P1-C 日界线 22:59/23:00/23:01、缺失时辰、六爻复现、真实用户写操作写保护、收藏/反馈持久化、普通/加密本机备份格式，共 27 项测试）
...
3. 设计并实现真太阳时和流派选择；子初换日已完成第一版，继续补默认策略与更多公开来源样例。
