...
| M1：确定性排盘核心 | 80% | 四类本地排盘、统一结果协议、输入边界、首批固定样例回归测试；P1-A 独立来源 Golden Case、八字设置字段和计算证据骨架已落地。 | 节气边界、全国城市库、真太阳时/子初换日/流派设置、节气/闰月/子初/夏令时等完整 Golden 回归测试。 |
...
- `npm run typecheck`
- `npm run lint`
- `npm test`（四模块固定样例、P1-A 独立来源 Golden Case、缺失时辰、跨 TZ 六爻复现、真实用户写操作写保护、收藏/反馈持久化、普通/加密本机备份格式，共 22 项测试）
