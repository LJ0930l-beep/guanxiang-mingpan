...
依据《项目复盘与三术解盘闭环开发执行书》（2026-09-08）完成任务包 R01–R10 的工程部分：

- [x] P0 数据可靠性与证据修复（记录容量、写盘事务、语义引用、冲突/平衡分离）。
- [x] 六爻三种起卦、紫微宫位联动、反馈自动关联、共享只读盘面、输入指纹与失效提示（R03/R05/R06/R07 工程部分）。
- [x] 八字基础大运与用户选定年份的流年对照（`bazi-dayun-v1`，工程基线，专业复核前不输出运势结论）。
- [x] 未知时辰八字部分盘（`bazi-partial-chart-policy.v1`）：年、月、日三柱与候选范围，时柱不补造；紫微维持阻止策略。
- [x] 生产依赖公告处置闭环（兼容升级 + 处置账本 + 门禁），高危公告 10 → 4 且全部登记。
- [x] 记录列表分页：1000 条记录可检索，展示层按 30 条/页加载。
- [ ] 大运/流年解释含义、流派选择与每术 30 个专业样例：待专业复核与负责人输入。
- [ ] 真实账号、真机/TestFlight、城市数据全国覆盖、生产部署与商店提交：外部条件（见 `docs/OWNER_DECISIONS_PENDING.md`）。
