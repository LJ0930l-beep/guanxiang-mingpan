...
## Phase 5 — 发布质量工程（P5-0 已完成，P5-A1 待派发）

Phase 5 以 [PROJECT_MASTER_EXECUTION.md](PROJECT_MASTER_EXECUTION.md) 为总执行账本，按一个小批一个小批推进，不把整阶段一次交给执行者。当前代码基线的真实事实仍以仓库、74/74 测试、Web 构建和 GitHub Actions 为准。

- [x] **P5-0 项目总账本与启动基线**：固化治理来源、Phase 0～4 真实基线、不可变产品边界、Level A/B、风险和批次验收模板。
- [ ] **P5-A 数据与计算可信度收口**：先做 P5-A1 城市覆盖/来源许可盘点与 Golden fixture 合同，再分批扩充全国城市和边界回归；未知地点不猜测，历史 Snapshot 不静默重算。
- [ ] **P5-B iPhone/TestFlight 设备验收**：完成真实设备上的四术、离线、无障碍和普通/加密备份文件流签字。
- [ ] **P5-C 本地隐私与用户控制材料**：完成首次使用说明、隐私政策、用户协议、注销/删除/导出边界。
- [ ] **P5-D 发布工程与质量门**：固定生产构建、商店材料、许可证清单、CI artifacts 和回滚规则。
- [ ] **P5-E 真实账号最小闭环（决策后）**：短信主账号、Apple/微信绑定、注销和审计；命盘默认不上传。
- [ ] **P5-F 跨平台权益接口（Level B）**：只同步身份与权益，不同步命盘。
- [ ] **P5-G 商业化接口（Level B）**：订阅/单次付费灰度，免费基础版长期保留。
- [ ] **P5-H 付费 AI 能力（Level B）**：仅服务端付费能力，完成成本、隐私和内容安全门。
- [ ] **P5-I 正式发布与运营复盘**：Level A 证据归档、灰度/回滚和首轮反馈准备后再决定公开提交。
