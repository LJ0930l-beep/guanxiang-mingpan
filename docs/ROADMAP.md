...
## Phase 5 — 发布质量工程（P5-0 已完成，P5-A1/P5-A2/P5-A3a/P5-A3b/P5-A4a/P5-A4b1/P5-A4b2/P5-A4b3/P5-A4b4/P5-A4b5/P5-A5a/P5-A5b/P5-A5c/P5-A final PASS；P5-C deferred route 功能尚未实现，下一批为 P5-B1）

当前收口记录：P5-A final 已由 Sol High/主管最终验收 PASS。`p5-a4a-cross-a11y-copy-route` 由独立 `p5-c-deferred-input-route.v1` 登记为 `status=deferred`、`disposition=routed-to-p5-c`、`implementationStatus=not-implemented`、`targetBatch=P5-C`；这只是路由，不代表 P5-C 功能已经实现。`p5-a4a-cross-city-coverage` 继续保持 P5-B 路由，下一批为 P5-B1 合同/来源/许可审计。本次 `npm audit --omit=dev` 生产依赖基线为 0 critical / 9 high / 16 moderate / 0 low（25 total），P5-E 仍需单独处置风险。
...
- [ ] **P5-B 城市数据完成**：完成中国大陆城市覆盖、来源/许可/别名/坐标审计和版本化离线数据；未知城市不猜测，历史 `locationId` 不静默替换。
  - [x] **P5-B1 城市数据合同、来源/许可审计与发布资格阻断门（实现完成，待主管独立验收）**：新增版本化、纯 JSON 可序列化的 `p5-b1-city-dataset-audit.v1` 合同、35 条现状审计 registry 与 fail-closed runtime validator；保留生产 35 条记录、35 个稳定 `locationId`、101 个名称/别名 token、解析行为和 `p5-a4a-cross-city-coverage` 的 P5-B 路由不变。当前快照明确 `status=partial`、`releaseEligibility=blocked`，逐行缺 `adminCode`/行政层级/坐标与别名来源证据，离线商业再分发许可为 `unknown`；候选来源与许可状态见 [DATASET_PROVENANCE.md](DATASET_PROVENANCE.md)。本批不扩充城市数据、不改 resolver/Storage Schema、不选择或宣称已获再分发许可；本地专项 8/8、统一回归 182/182、typecheck、lint、Web Export 8 routes 已通过，生产依赖审计与远端 CI 证据待本批收口。
