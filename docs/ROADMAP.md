...
## Phase 5 — 发布质量工程（P5-0 已完成，P5-A1 独立验收 PASS）

Phase 5 以 [PROJECT_MASTER_EXECUTION.md](PROJECT_MASTER_EXECUTION.md) 为总执行账本，按一个小批一个小批推进，不把整阶段一次交给执行者。P5-A1 的已验收基线为 83/83；P5-A2 实现后预期为 87 项，最终事实以主管复跑、Web 构建和 GitHub Actions 为准。

- [x] **P5-0 项目总账本与启动基线**：固化治理来源、Phase 0～4 真实基线、不可变产品边界、Level A/B、风险和批次验收模板。
- [ ] **P5-A 专业质量补强（四术 Golden/边界/输入策略）**：P5-A1 已通过主管独立验收；P5-A2 已完成实现但等待主管验收，整个 P5-A 未完成。不得伪造专业真值。
  ...
  - [ ] **P5-A2 香港天文台 published-reference Golden**：已完成实现，新增 2 条 HKO 公开资料 fixture 与 4 项离线测试；立春只按公开分钟比较，农历只按公开日期比较；等待 Sol High 独立验收，不能把整个 P5-A 标为完成。
  - [ ] **P5-A3 真太阳时来源标签决策门（候选）**：登记 `TRUE_SOLAR_DATA_VERSION` 与当前公式来源不一致、且常量未进入保存 evidence 的风险；由主管决定继续近似式并纠正标签，或切换 NOAA 公式并处理版本/历史兼容，未授权实现。
