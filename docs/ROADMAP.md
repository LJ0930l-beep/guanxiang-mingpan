...
- [ ] **P5-A4b3 八字真太阳时跨日/子初边界矩阵与 cumulative overlay（实现完成待 Sol High 独立验收）**：本小批只关闭 `p5-a4a-bazi-true-solar-cross-day`；新增 regression-only 135°E / 75°E（相对 120°E 标准经线两侧）、正负应用修正、民用向前/向后跨日与 `midnight`/`ziEarly` 矩阵，锁定真太阳时民用/有效时刻和最终 `effectiveCalculationTime`。新增 `p5-a4b-input-resolution.v3` 共 6 项（v1 原 3 + v2 追加 2 + 本批八字 1），v1/v2 原 exports、registry、validator、顺序与精确 3/5 计数保持不变；version-aware validator 保持纯 JSON、唯一 ID、原始 gap/P5-A4b 关联且不写 commit SHA。矩阵在 UTC/Asia/Shanghai 下整体 deepEqual，A4a 41/18/15/5/2/1 与 astrology `0,0` probe 保持；不修改算法/公式/日界线或处理其他 gap/owner decision。已核对 Expo SDK 57 exact docs，本批没有使用 Expo API；Sol High 尚未独立验收。
