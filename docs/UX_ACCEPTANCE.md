...
> 版本：2026-09-08。自动化证据与人工 viewport 记录分开；没有真实 iPhone 时不把 Web 结果写成设备 PASS。
...
`tests/p5-c-page-accessibility.regression.mjs` 11/11、`npm test` 239/239；记录页非嵌套 Pressable、未知 slug、四模块具体深层路由、future-schema 只读提示均有回归断言。当前代码尚未推送远端，因此不引用旧 Actions 作为本轮证据。Web 375×812、768×1024、1024×768、1440×900 的手工矩阵，以及 iPhone/VoiceOver 的真实设备矩阵，仍需外部验收记录，不能从静态或 CUA 证据推导 PASS。
