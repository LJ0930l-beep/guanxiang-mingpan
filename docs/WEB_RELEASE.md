...
> 版本：2026-09-08。当前为部署前交付；没有域名、托管账号和公开隐私 URL，不宣称已上线。
...
静态路由：`/`、`/home`、`/profiles`、`/records`、`/settings`、`/privacy`、`/terms`、`/module/[slug]`、`/module/bazi`、`/module/liuyao`、`/module/ziwei`、`/module/astrology`、`/_sitemap`、`/+not-found`（共 14 条）。四个具体模块路由各自生成静态 HTML，支持直接打开和刷新；实际托管仍需验证无扩展名 URL 的 rewrite/clean URL 规则。
...
- `sw.js` 是 network-first：成功响应会写入同源运行时缓存，导航失败回退对应缓存/离线页，非 HTML 资源离线返回 503；当前 `guanxiang-shell-v2` 会在 activate 时删除旧前缀缓存，避免发布旧 bundle。
...
本地 `npm run build:web` 与 `npm run verify:web` 已通过，导出并验证 14 条路由及全部 public release files；统一 `npm test` 为 239/239。四个具体模块 HTML 和 `sw.js` 已通过本地 HTTP 200 直达检查；当前代码尚未推送远端，因此没有本轮 GitHub Actions 绿色结论。仍缺正式域名、HTTPS 托管、公开支持/隐私 URL、真实浏览器兼容签字和部署回滚负责人，因此本文件是部署前交付，不是上线证明。
