...
def build():
    ...
    add_body(doc, "本文记录本轮将观象命盘从技术 RC 补到本地排盘、证据、解释、保存、历史查看和事实反馈闭环的结果。八字、六爻、紫微三术都有保存时的结构化盘面、规则版本和解释快照。基础规则计算仍在设备本地，未接入广告、支付、云同步、真实身份供应商或生产 AI。")
    add_body(doc, "结论：当前仓库可完成的 R01–R10 已实现并经过本地回归；正式公开发布仍受真机与签名、主体与商店合规、城市数据许可、传统术数专业复核、生产依赖漏洞处置和 Qwen 离线评测阻塞。")

    add_heading(doc, "一 交付结论", 1)
    add_body(doc, "本轮已生成本地提交，当前尚未推送远端；具体 SHA 以最终 git rev-parse HEAD 记录为准。当前不能声称本轮 GitHub Actions 为绿色，也不引用旧 run 作为本轮证据。推送后必须检查该 SHA 对应的 Actions，确认 Web Export 与 Verify Web Export 实际执行并成功。")
    ...
    matrix = [
        ...
        ("F06", "部分完成", "八字保留主题、条件、反证、候选取用、证据链和未知时辰 fail-closed；大运/流年深度事实引擎仍待专业规则确认。"),
        ("F07", "完成 基础规则", "六爻支持快捷自动、六次三枚铜钱投掷、手工录入；实现 1:3:3:1 的 6/7/8/9 规则，保留用神、世应、旺衰、动变和变后事实，不承诺确定应期。"),
        ("F08", "完成 基础深度", "紫微十二宫、命身定位、主星/亮度、四化、对宫/三方四正节点可回查；占星支持全量相位与日级近似边界。"),
        ("F09", "完成", "实时页和历史页复用只读 ChartRenderer；旧记录缺数组时安全降级不重算；反馈默认不关联，用户选择具体解读卡及其相关证据并保持导入导出关联。"),
        ("F10", "部分完成", "安全扫描、生产 audit、Web 14 路由导出、四模块深链和 Service Worker 缓存迁移已收口；真机、证书、主体、许可与线上部署仍阻塞。"),
        ("F11", "方案完成", "Qwen3.5-9B 只形成脱敏离线评测方案，不接生产、不上传真实档案、不购买算力；权重和人工评测未执行。"),
    ...
    add_body(doc, "首页 → 命主 → 八字 → 选择性别、日界线、真太阳时 → 排出四柱 → 查看主题判断、条件/反证、候选取用和计算依据 → 保存到记录 → 记录页打开保存时完整盘面、解释和事实反馈。未知时辰不补造时柱，也不把大运/流年缺失事实写成确定结论。")
    add_heading(doc, "六爻", 2)
    add_body(doc, "首页 → 六爻 → 输入至少 4 字问题 → 选择用神方向 → 选择快捷自动、六次三枚铜钱投掷或手工录入 → 逐条检查爻位、世应、旺衰、动静、纳甲和 6/7/8/9 点 → 生成解释并保存。保存失败时点击重试保存原卦，不会重新随机。")
    add_heading(doc, "紫微", 2)
    add_body(doc, "首页 → 紫微 → 生成十二宫 → 点击宫位查看主星、亮度、命身定位、对宫和三方四正 → 展开四化和主题解释 → 记录页查看保存时完整十二宫。命宫文案使用真实干支坐标，不再输出无信息的命宫落在命宫。")
    add_heading(doc, "占星与历史", 2)
    add_body(doc, "首页 → 星盘 → 生成本命盘 → 查看全部星体、宫位和相位；未知时辰只展示通过全天稳定性检查的日级落座。历史页只读保存时结果，修订规则需要用户主动运行当前规则复核并生成 Diff。")
    ...
    checks = [
        ("npm test", "通过 239/239，0 失败，0 跳过", "含六爻三枚铜钱、旧档案 render、反馈关联、R01 storage、R02 语义引用、R03 指纹和 R06 紫微关系回归"),
        ("npm run typecheck", "通过", "TypeScript 无错误"),
        ("npm run lint", "通过", "Expo lint 无错误"),
        ("npm run build:web", "通过", "真实导出 14 个静态路由，含四模块具体深链"),
        ("npm run verify:web", "通过", "14 routes、公共发布文件、bundle、深链和 SW 迁移验证通过"),
    ...
    add_body(doc, "生产漏洞主要来自 Expo、Metro、Router、xcode 等传递依赖。本轮逐项记录了可达性和 SDK 57 兼容性，没有执行 npm audit fix --force，也没有把漏洞白名单当作清零。")
    ...
    for item in [
        ...
        "八字大运/流年、紫微流派与四化、六爻细分规则需要专业审核和可引用来源。",
    ...
    add_body(doc, "工程交接：docs/PROJECT_REVIEW_HANDOFF_20260908.md。生产依赖审计：docs/PRODUCTION_AUDIT_20260908.md。Qwen 离线方案：docs/QWEN35_9B_OFFLINE_EVAL_PLAN_20260908.md。本轮本地提交尚未推送远端；CI run 和最终验收以本轮实际记录为准。")
