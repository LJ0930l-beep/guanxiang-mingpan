...
import { buildRelationEvidenceNodes } from '@/domains/bazi/evidence/relation-evidence';
import { buildStrengthAssessment } from '@/domains/bazi/evidence/support-control-evidence';

export type { BaziEvidenceGraph, EvidenceNode, EvidenceSource, EvidenceWeight, StrengthAssessment, StrengthConfidence, StrengthDecisionStep, StrengthStatus } from '@/domains/bazi/evidence/evidence-types';
...
export function buildBaziEvidenceGraph(
  ...
  const nodes = [
    ...
    ...buildRelationEvidenceNodes(chart),
  ];
  const graph: BaziEvidenceGraph = {
    ...
    nodes,
    relationEdges: chart.relations,
  };
  graph.strengthAssessment = buildStrengthAssessment(chart, nodes);
  return graph;
}
